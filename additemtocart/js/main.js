	...
	app.createProducts = function(){
		...
		for(var i = 0; i < productos.length; i++){
			...
			if(productos[i].stock > 0){
				...
				contenido+= '		</span>'
				contenido+= '		<a class="large-12 columns btn submit ladda-button prod-'+productos[i].id+'" data-style="slide-right" onclick="app.addtoCart('+productos[i].id+');">Add to basket</a>'
	...
	app.searchProd = function(cart,id,cant,name,price,img,available){
		...
		if(undefined != curProd && curProd != null){
			...
			if(curProd.cant < available){
				...
				alert('You cannot add more of this product')
			}
